      ...
      <div class="card-body">
        ...
        <table class="table table-hover">
          ...
          <tbody>
            ...
            <tr v-else v-for="(rating, index) in ratings.data" :key="index">
              ...
              <td>
                <a
                  href="#"
                  class="text-primary"
                  @click="viewDetailUser(rating)"
                >
                  {{ rating.user.name }}
                </a>
              </td>
              <td>
                <p>{{ rating.partner.name }}</p>
              </td>
              <td>{{ rating.created_at | myDate }}</td>
              <td class="text-center">
                <a href="#" @click="deleteModal(rating)">
        ...
        <paginate
          :page-count="ratings.last_page == null ? 1 : ratings.last_page"
...
export default {
  ...
  created() {
    store.dispatch("rating/RATINGS_FETCH");
  },
  computed: {
    ...mapGetters("rating", ["ratings", "errors", "isLoading"])
  },
  methods: {
    ...mapActions("rating", ["RATING_DELETE", "RATINGS_PAGINATE"]),
    ...
    async submitDelete() {
      ...
      try {
        await this.RATING_DELETE(this.id);
    ...
    viewDetailUser(rating) {
      this.USER_ONE_FETCH(rating.user.id).then(({ data }) => {
        this.user = data;
